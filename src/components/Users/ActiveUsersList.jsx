...
const ActiveUsersList = ({ onUserSelect }) => {
  const { users, loading, error, refresh } = useActiveUsers();
  ...
  const filteredUsers = users.filter(user => {
    ...
    return (
      ...
      user.dpi?.toString().includes(searchTerm) ||
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <div className="relative max-w-md flex-1 mr-4">
          ...
          <input
            type="text"
            placeholder="Buscar usuarios por nombre, usuario, cuenta, etc..."
      ...
      <p className="text-sm text-gray-500">
        Mostrando {filteredUsers.length} de {users.length} usuarios activos
      </p>
      ...
      {filteredUsers.length === 0 ? (
        <div className="bg-white rounded-lg shadow p-8 text-center">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">
            {searchTerm ? 'No se encontraron usuarios' : 'No hay usuarios activos'}
          </h3>
          {searchTerm && (
            <button
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">DPI</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredUsers.map((user) => (
                <tr key={user._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-500">{user.dpi}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-500">{user.companyName}</div>
                    <div className="text-xs text-gray-400">Ingresos: Q{user.income}</div>
                  </td>
