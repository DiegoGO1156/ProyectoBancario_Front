  ...
  return (
    <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl p-6 shadow-sm border border-blue-200">
      ...
      <div className="grid grid-cols-3 gap-4">
        {goals.map((goal) => (
          <button key={goal.id} className="group bg-white p-4 rounded-xl shadow-xs border border-blue-200 hover:border-blue-300 hover:-translate-y-1 transition-all flex flex-col items-center">
            <div className="bg-blue-100 p-3 rounded-full mb-2 group-hover:bg-blue-200 transition-colors">
              {goal.icon}
            </div>
            <span className="text-sm font-medium text-blue-800 group-hover:text-blue-900">{goal.title}</span>
          </button>
        ))}
      </div>
    </div>
